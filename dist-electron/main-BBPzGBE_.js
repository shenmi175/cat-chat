...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    // Explicitly set ARGB transparent background
    frame: false,
    // Frameless window
    alwaysOnTop: true,
    // Always on top
    ...
    hasShadow: false,
    webPreferences: {
      // vite-plugin-electron builds preload as .mjs in dev, .js in prod
      preload: (() => {
        const mjs = require$$1$1.join(__dirname$1, "preload.mjs");
        const js = require$$1$1.join(__dirname$1, "preload.js");
...
ipcMain.handle("get-system-state", async () => {
  try {
    const activeWin = (await import("./index-BRX90hJz.js")).default;
    const activeWindow = await activeWin();
    const battery2 = await si.battery();
    return {
      activeWindow: activeWindow ? activeWindow.title : "未知窗口",
      activeApp: activeWindow ? activeWindow.owner.name : "未知应用",
      batteryPercent: battery2.percent,
      isCharging: battery2.isCharging,
